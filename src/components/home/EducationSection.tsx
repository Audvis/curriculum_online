...
export function EducationSection({ education }: { education: Education[] }) {
  ...
  return (
    <section id="education" className="py-12 sm:py-16 md:py-20 relative z-10">
          ...
          {education.map((edu, index) => (
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-3 sm:mb-4">
                <div>
                  <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-1 sm:mb-2">{edu.degree}</h3>
                  <p className="text-base sm:text-lg md:text-xl text-blue-300">{edu.institution}</p>
                  {edu.field && <p className="text-gray-300 text-sm sm:text-base">{edu.field}</p>}
                </div>
              ...
              {edu.description && <p className="text-gray-300 leading-relaxed text-sm sm:text-base">{edu.description}</p>}
            </motion.div>
          ))}
        </div>
      </div>
